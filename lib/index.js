...
function middleware(req, res, next) {
    ...
    res.locals.etupay = {};
    res.locals.etupay.paid = (payload.step === 'paid' || payload.step === 'authorization');
    res.locals.etupay.step = payload.step;
    res.locals.etupay.transactionId = payload.transaction_id;
    res.locals.etupay.serviceData = payload.service_data;
...
module.exports = (config_) => {
    if (!config_.id || !config_.key || !config_.url) {
        throw new Error('Config must have id, url, and key');
    }
